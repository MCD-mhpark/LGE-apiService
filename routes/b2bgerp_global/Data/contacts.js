...
function B2B_GERP_GLOBAL_ENTITY() {
	...
	this.ATTRIBUTE_24 = "";		//Customer Type
	//this.ATTRIBUTE_25 = "";		//Customer Sub Type 사용 예정
	this.ATTRIBUTE_26 = "";		//Direct Marketing_EM YN
	this.ATTRIBUTE_27 = "";		//Direct Marketing_EM Date
...
async function Convert_B2BGERP_GLOBAL_DATA(contacts_data, business_department) {
	...
	for (var i = 0; i < contacts_data.elements.length; i++) {
		...
		try {
			...
			result_item.ATTRIBUTE_24 = GetBusiness_Department_data(FieldValues_data, business_department, "Customer Type");    //Customer Type
			//result_item.ATTRIBUTE_25 = Customer Sub Type 사용예정
			
			result_item.ATTRIBUTE_26 = GetCustomFiledValue(FieldValues_data, 100211) == "Yes" ? "Y" : "N";    //Direct Marketing_EM YN
			var Direct_Marketing_EM_Date = utils.timeConverter("GET_DATE", GetCustomFiledValue(FieldValues_data, 100200));
			result_item.ATTRIBUTE_27 = Direct_Marketing_EM_Date == null ? "" : Direct_Marketing_EM_Date;    //Direct Marketing_EM Date
...
function CONVERT_B2BGERP_GLOBAL_CUSTOMOBJECT(request_data) {
	...
	for (const item of request_data) {
		...
		mql_data.fieldValues.push({
			"id": "1670",
			"value": item.ATTRIBUTE_24
		})

		mql_data.fieldValues.push({
			"id": "1983",
			"value": item.ATTRIBUTE_26
		})
